...
Neuron::Neuron(unsigned numOutputs, unsigned myIndex, vector<double>& input_vals ,int& line, int neuron) {
	...
	for (int i = 0; i < numOutputs; i++)
	{
		...
		{
			outputWeights.back().weight = input_vals[line-1];
		}
		line++;
	}
...
{
public:
	void InitializeComponent(const vector<unsigned>& topology, int idd);
...
void NN::InitializeComponent(const vector<unsigned>& topology, int idd)
{
	id = idd;
